...
class Consent_Manager {
	...
	public function sanitize_record( array $payload ) {
		...
		if ( null === $categories ) {
			return new \WP_Error( 'pcm_invalid_payload', __( 'Missing consent categories.', 'privacy-consent-manager' ), array( 'status' => 400 ) );
		}

		$anonymous_id = isset( $payload['anonymous_id'] ) ? (string) $payload['anonymous_id'] : '';
		if ( '' !== $anonymous_id && ! wp_is_uuid( $anonymous_id ) ) {
			return new \WP_Error( 'pcm_invalid_id', __( 'Invalid anonymous identifier.', 'privacy-consent-manager' ), array( 'status' => 400 ) );
		}
	...
	public function get_client_config() {
		...
		return array(
			...
			'language'       => get_locale(),
			'i18n'           => array(
				'preferencesTitle' => __( 'Privacy Preferences', 'privacy-consent-manager' ),
				'alwaysActive'     => __( 'Always Active', 'privacy-consent-manager' ),
				'privacyPolicy'    => __( 'Privacy Policy', 'privacy-consent-manager' ),
				'cookiePolicy'     => __( 'Cookie Policy', 'privacy-consent-manager' ),
				'close'            => __( 'Close', 'privacy-consent-manager' ),
				'consentId'        => __( 'Consent ID', 'privacy-consent-manager' ),
			),
		);
	}
}
