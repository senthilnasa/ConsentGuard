...
class Rest_Api {
	...
	public function admin_permission() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return new \WP_Error(
				'pcm_forbidden',
				__( 'You are not allowed to do that.', 'privacy-consent-manager' ),
	...
	public function consent_rate_limit() {
		...
		if ( $count >= 20 ) {
			return new \WP_Error(
				'pcm_rate_limited',
				__( 'Too many consent submissions. Please try again shortly.', 'privacy-consent-manager' ),
				array( 'status' => 429 )
			);
		}
	...
	public function update_settings( \WP_REST_Request $request ) {
		...
		if ( ! is_array( $body ) || empty( $body ) ) {
			return new \WP_Error( 'pcm_empty', __( 'No settings provided.', 'privacy-consent-manager' ), array( 'status' => 400 ) );
		}
