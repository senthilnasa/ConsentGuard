...
class Privacy_Manager {
	...
	public function register_eraser( $erasers ) {
		$erasers['pcm-consents'] = array(
			'eraser_friendly_name' => __( 'Consent records (Privacy & Consent Manager)', 'privacy-consent-manager' ),
	...
	public function erase_consents( $identifier ) {
		...
		return array(
			...
			'messages'       => $removed > 0
				? array( __( 'Consent records associated with the provided consent ID were deleted.', 'privacy-consent-manager' ) )
				: array( __( 'No consent records matched the provided identifier. Note: consent records are stored under an anonymous ID, not an email address.', 'privacy-consent-manager' ) ),
			'done'           => true,
		);
	}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p>' . esc_html__( 'This website uses Privacy & Consent Manager to obtain and store visitor consent for cookies and similar technologies. When a visitor makes a consent choice, an anonymous record (random identifier, timestamp, chosen categories, consent version, region code and language) is stored. No IP addresses are stored by this plugin.', 'privacy-consent-manager' ) . '</p>'
			. '<p>' . esc_html__( 'Consent records are retained for the configured retention period and then deleted automatically.', 'privacy-consent-manager' ) . '</p>';
		wp_add_privacy_policy_content( __( 'Privacy & Consent Manager', 'privacy-consent-manager' ), wp_kses_post( $content ) );
	}
