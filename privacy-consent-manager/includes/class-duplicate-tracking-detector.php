...
class Duplicate_Tracking_Detector {
	...
	public function signatures() {
		return array(
			'ga4'        => array(
				'label'    => __( 'Google Analytics 4', 'privacy-consent-manager' ),
				'patterns' => array(
			...
			'gtm'        => array(
				'label'    => __( 'Google Tag Manager', 'privacy-consent-manager' ),
				'patterns' => array(
			...
			'clarity'    => array(
				'label'    => __( 'Microsoft Clarity', 'privacy-consent-manager' ),
				'patterns' => array(
			...
			'cloudflare' => array(
				'label'    => __( 'Cloudflare Web Analytics', 'privacy-consent-manager' ),
				'patterns' => array(
					'#static\.cloudflareinsights\.com/beacon(?:\.min)?\.js#i',
				),
			),
			'meta_pixel' => array(
				'label'    => __( 'Meta Pixel', 'privacy-consent-manager' ),
				'patterns' => array(
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Could not fetch the homepage for scanning: %s', 'privacy-consent-manager' ),
		...
		if ( $code < 200 || $code >= 400 ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Homepage scan returned HTTP status %d.', 'privacy-consent-manager' ),
					$code
				)
			);
		}
	...
	private function attribute_sources( $service, array $active_plugins ) {
		...
		if ( ! empty( $ours[ $service ] ) ) {
			$sources[] = __( 'Privacy & Consent Manager', 'privacy-consent-manager' );
		}
