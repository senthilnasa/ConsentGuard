...
$pcm_positions  = array(
	'header' => __( 'Header', 'privacy-consent-manager' ),
	'body'   => __( 'Body (after opening body tag)', 'privacy-consent-manager' ),
	'footer' => __( 'Footer', 'privacy-consent-manager' ),
);

Admin::maybe_notice();

if ( ! current_user_can( 'unfiltered_html' ) ) {
	echo '<div class="notice notice-warning"><p>' . esc_html__( 'Your account lacks the unfiltered_html capability: script tags you save will be stripped to safe HTML. Ask a super administrator to add executable scripts.', 'privacy-consent-manager' ) . '</p></div>';
}
?>
<h1><?php esc_html_e( 'Script Manager', 'privacy-consent-manager' ); ?></h1>
<p><?php esc_html_e( 'Custom scripts are printed in consent-blocked form and execute only after the visitor grants the selected category. Empty the name to delete a script.', 'privacy-consent-manager' ); ?></p>
...
foreach ( $pcm_rows as $pcm_i => $pcm_script ) :
	...
	<div class="pcm-card pcm-script-row">
		<h2><?php echo $pcm_is_new ? esc_html__( 'Add Script', 'privacy-consent-manager' ) : esc_html( $pcm_script['name'] ); ?></h2>
		<input type="hidden" name="pcm[custom_scripts][<?php echo esc_attr( $pcm_i ); ?>][id]" value="<?php echo esc_attr( $pcm_script['id'] ); ?>" />
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Name', 'privacy-consent-manager' ); ?></th>
				<td><input type="text" class="regular-text" name="pcm[custom_scripts][<?php echo esc_attr( $pcm_i ); ?>][name]" value="<?php echo esc_attr( $pcm_script['name'] ); ?>" placeholder="<?php esc_attr_e( 'e.g. Facebook Pixel', 'privacy-consent-manager' ); ?>" /></td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Category', 'privacy-consent-manager' ); ?></th>
				<td>
			...
			<tr>
				...
				<td>
					...
					<p class="description"><?php esc_html_e( 'Inline snippets, external script tags and tracking pixels are supported.', 'privacy-consent-manager' ); ?></p>
				</td>
			</tr>
		</table>
	</div>
