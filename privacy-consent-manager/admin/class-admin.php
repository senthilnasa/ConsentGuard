...
/**
 * Registers the "Privacy & Consent" menu, renders view files and processes
...
class Admin {
	...
	public function register_menu() {
		$this->pages = array(
			'pcm-dashboard'     => array(
				'title' => __( 'Dashboard', 'privacy-consent-manager' ),
				'view'  => 'dashboard',
			),
			'pcm-banner'        => array(
				'title' => __( 'Consent Banner', 'privacy-consent-manager' ),
				'view'  => 'banner',
			),
			'pcm-categories'    => array(
				'title' => __( 'Consent Categories', 'privacy-consent-manager' ),
				'view'  => 'categories',
			),
			'pcm-analytics'     => array(
				'title' => __( 'Analytics', 'privacy-consent-manager' ),
				'view'  => 'analytics',
			),
			'pcm-scripts'       => array(
				'title' => __( 'Script Manager', 'privacy-consent-manager' ),
				'view'  => 'scripts',
			),
			'pcm-conflicts'     => array(
				'title' => __( 'Plugin Conflicts', 'privacy-consent-manager' ),
				'view'  => 'conflicts',
			),
			'pcm-scanner'       => array(
				'title' => __( 'Cookie/Script Scanner', 'privacy-consent-manager' ),
				'view'  => 'scanner',
			),
			'pcm-jurisdictions' => array(
				'title' => __( 'Jurisdiction Rules', 'privacy-consent-manager' ),
				'view'  => 'jurisdictions',
			),
			'pcm-records'       => array(
				'title' => __( 'Consent Records', 'privacy-consent-manager' ),
				'view'  => 'records',
			),
			'pcm-policies'      => array(
				'title' => __( 'Privacy Policies', 'privacy-consent-manager' ),
				'view'  => 'policies',
			),
			'pcm-settings'      => array(
				'title' => __( 'Settings', 'privacy-consent-manager' ),
				'view'  => 'settings',
			),
			'pcm-tools'         => array(
				'title' => __( 'Tools', 'privacy-consent-manager' ),
				'view'  => 'tools',
			),
		);

		add_menu_page(
			__( 'Privacy & Consent', 'privacy-consent-manager' ),
			__( 'Privacy & Consent', 'privacy-consent-manager' ),
			Security::CAP_MANAGE,
			'pcm-dashboard',
	...
	public function render_page() {
		if ( ! current_user_can( Security::CAP_MANAGE ) ) {
			wp_die( esc_html__( 'You are not allowed to access this page.', 'privacy-consent-manager' ), 403 );
		}
		...
		if ( is_readable( $file ) ) {
			...
			echo '<h1>' . esc_html__( 'Privacy & Consent Manager', 'privacy-consent-manager' ) . '</h1>';
			echo '<p>' . esc_html__( 'View not found.', 'privacy-consent-manager' ) . '</p>';
		}
		echo '</div>';
	}
	...
	public static function form_close() {
		submit_button( __( 'Save Changes', 'privacy-consent-manager' ) );
		echo '</form>';
	}
	...
	public static function maybe_notice() {
		...
		if ( isset( $_GET['pcm-saved'] ) ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Settings saved.', 'privacy-consent-manager' ) . '</p></div>';
		}
		if ( isset( $_GET['pcm-notice'] ) ) {
			$key      = sanitize_key( wp_unslash( $_GET['pcm-notice'] ) );
			$messages = array(
				'done'             => array( 'success', __( 'Conflict updated.', 'privacy-consent-manager' ) ),
				'manual'           => array( 'warning', __( 'Automatic conflict resolution is not available for this plugin. Please disable the tracking feature manually in that plugin\'s settings.', 'privacy-consent-manager' ) ),
