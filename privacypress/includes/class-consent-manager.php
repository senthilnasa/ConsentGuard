...
class Consent_Manager {
	...
	public function sanitize_record( array $payload ) {
		...
		if ( null === $categories ) {
			return new \WP_Error( 'pcm_invalid_payload', __( 'Missing consent categories.', 'privacypress' ), array( 'status' => 400 ) );
		}

		$anonymous_id = isset( $payload['anonymous_id'] ) ? (string) $payload['anonymous_id'] : '';
		if ( '' !== $anonymous_id && ! wp_is_uuid( $anonymous_id ) ) {
			return new \WP_Error( 'pcm_invalid_id', __( 'Invalid anonymous identifier.', 'privacypress' ), array( 'status' => 400 ) );
		}
