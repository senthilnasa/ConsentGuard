...
Admin::maybe_notice();
?>
<h1><?php esc_html_e( 'Cookie / Script Scanner', 'privacypress' ); ?></h1>
...
<p>
	<a class="button button-primary" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=pcm_scan' ), 'pcm_scan', '_pcm_nonce' ) ); ?>">
		<?php esc_html_e( 'Scan Now', 'privacypress' ); ?>
	</a>
	<?php if ( ! empty( $pcm_scan['time'] ) ) : ?>
		<span class="description">
			...
			printf(
				...
				esc_html__( 'Last scan: %s ago', 'privacypress' ),
...
<p class="description"><?php esc_html_e( 'The scanner fetches your own homepage and inspects it for known trackers and third-party scripts. It runs only when you click Scan Now.', 'privacypress' ); ?></p>

<?php if ( ! empty( $pcm_scan['results'] ) ) : ?>
	<h2><?php esc_html_e( 'Duplicate Tracking', 'privacypress' ); ?></h2>
	<table class="widefat striped pcm-table">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Service', 'privacypress' ); ?></th>
				<th><?php esc_html_e( 'Instances on page', 'privacypress' ); ?></th>
				<th><?php esc_html_e( 'Configured by', 'privacypress' ); ?></th>
				<th><?php esc_html_e( 'Status', 'privacypress' ); ?></th>
			</tr>
		</thead>
		<tbody>
			...
			<tr>
				...
				<td>
					<?php if ( ! empty( $pcm_result['duplicate'] ) ) : ?>
						<span class="pcm-badge pcm-badge-amber">⚠ <?php esc_html_e( 'Potential duplicate collection', 'privacypress' ); ?></span>
					...
					<?php elseif ( $pcm_result['instances'] > 0 || $pcm_result['sources'] ) : ?>
						<span class="pcm-badge pcm-badge-green"><?php esc_html_e( 'OK', 'privacypress' ); ?></span>
					<?php else : ?>
						<span class="pcm-badge"><?php esc_html_e( 'Not detected', 'privacypress' ); ?></span>
					<?php endif; ?>
				</td>
			</tr>
		<?php endforeach; ?>
...
<?php if ( ! empty( $pcm_scan['scripts'] ) ) : ?>
	...
	<table class="widefat striped pcm-table">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Domain', 'privacypress' ); ?></th>
				<th><?php esc_html_e( 'Category', 'privacypress' ); ?></th>
				<th><?php esc_html_e( 'Classify', 'privacypress' ); ?></th>
			</tr>
		</thead>
		<tbody>
			...
			<tr>
				...
				<td>
					<?php if ( 'unknown' === $pcm_script['category'] ) : ?>
						<span class="pcm-badge pcm-badge-amber">⚠ <?php esc_html_e( 'Unknown Tracker', 'privacypress' ); ?></span>
					<?php else : ?>
				...
				<td>
					<select name="pcm[scanner][classifications][<?php echo esc_attr( $pcm_script['host'] ); ?>]">
						<option value="unknown" <?php selected( $pcm_script['category'], 'unknown' ); ?>><?php esc_html_e( 'Unknown', 'privacypress' ); ?></option>
	...
	<p class="description"><?php esc_html_e( 'Unknown trackers are not assumed harmless. Classifying a domain adds it to the script blocker so it is held until the visitor grants that category.', 'privacypress' ); ?></p>
