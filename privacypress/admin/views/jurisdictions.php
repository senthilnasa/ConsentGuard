...
Admin::maybe_notice();
?>
<h1><?php esc_html_e( 'Jurisdiction Rules', 'privacypress' ); ?></h1>

<?php Admin::form_open( array( 'jurisdictions', 'dpdp' ) ); ?>

<table class="form-table" role="presentation">
	<tr>
		<th scope="row"><?php esc_html_e( 'Geolocation', 'privacypress' ); ?></th>
		<td>
			<label><input type="hidden" name="pcm[jurisdictions][geo_enabled]" value="" /><input type="checkbox" name="pcm[jurisdictions][geo_enabled]" value="1" <?php checked( $pcm_j['geo_enabled'] ); ?> /> <?php esc_html_e( 'Resolve the visitor country from trusted infrastructure headers (Cloudflare, GeoIP modules)', 'privacypress' ); ?></label>
			<p class="description"><?php esc_html_e( 'No external geolocation API is called and no IP address is stored. When disabled — or when no header is available — every visitor gets the default profile, so keep the default profile the strictest one you need. Note: with full-page caching, per-country behaviour requires your cache to vary on the country header.', 'privacypress' ); ?></p>
		</td>
	</tr>
	<tr>
		<th scope="row"><?php esc_html_e( 'Default profile', 'privacypress' ); ?></th>
		<td>
...
<h2><?php esc_html_e( 'Country → Profile rules', 'privacypress' ); ?></h2>
<table class="widefat striped pcm-table" style="max-width:560px">
	<thead>
		<tr>
			<th><?php esc_html_e( 'Country code (ISO 3166-1 alpha-2)', 'privacypress' ); ?></th>
			<th><?php esc_html_e( 'Profile', 'privacypress' ); ?></th>
		</tr>
	</thead>
	<tbody>
	<?php
...
<p class="description"><?php esc_html_e( 'EU/EEA countries automatically map to the GDPR profile unless overridden. Clear a country code to remove its rule.', 'privacypress' ); ?></p>

<h2><?php esc_html_e( 'Profiles', 'privacypress' ); ?></h2>
<table class="widefat striped pcm-table" style="max-width:760px">
	<thead>
		<tr>
			<th><?php esc_html_e( 'Profile', 'privacypress' ); ?></th>
			<th><?php esc_html_e( 'Consent model', 'privacypress' ); ?></th>
			<th><?php esc_html_e( 'Require consent before non-essential tracking', 'privacypress' ); ?></th>
			<th><?php esc_html_e( 'Show Reject All', 'privacypress' ); ?></th>
			<th><?php esc_html_e( 'Granular categories', 'privacypress' ); ?></th>
		</tr>
	</thead>
	<tbody>
	<?php foreach ( $pcm_profiles as $pcm_key => $pcm_profile ) : ?>
		<tr>
			...
			<td>
				<select name="pcm[jurisdictions][profiles][<?php echo esc_attr( $pcm_key ); ?>][mode]">
					<option value="opt_in" <?php selected( $pcm_profile['mode'] ?? 'opt_in', 'opt_in' ); ?>><?php esc_html_e( 'Opt-in (consent first)', 'privacypress' ); ?></option>
					<option value="opt_out" <?php selected( $pcm_profile['mode'] ?? 'opt_in', 'opt_out' ); ?>><?php esc_html_e( 'Opt-out (implied until objection)', 'privacypress' ); ?></option>
					<option value="notice_only" <?php selected( $pcm_profile['mode'] ?? 'opt_in', 'notice_only' ); ?>><?php esc_html_e( 'Notice only', 'privacypress' ); ?></option>
				</select>
			</td>
...
<h2><?php esc_html_e( 'India / DPDP Configuration', 'privacypress' ); ?></h2>
<p class="description"><?php esc_html_e( 'These texts are shown to visitors matched to the DPDP profile. Nothing is hard-coded — adapt the wording to your organization\'s actual data processing and legal advice.', 'privacypress' ); ?></p>
<table class="form-table" role="presentation">
	<tr>
		<th scope="row"><label for="pcm-dpdp-notice"><?php esc_html_e( 'Notice text', 'privacypress' ); ?></label></th>
	...
	<tr>
		<th scope="row"><label for="pcm-dpdp-purpose"><?php esc_html_e( 'Purpose descriptions', 'privacypress' ); ?></label></th>
	...
	<tr>
		<th scope="row"><label for="pcm-dpdp-rights"><?php esc_html_e( 'User rights information', 'privacypress' ); ?></label></th>
	...
	<tr>
		<th scope="row"><label for="pcm-dpdp-contact"><?php esc_html_e( 'Contact email', 'privacypress' ); ?></label></th>
	...
	<tr>
		<th scope="row"><label for="pcm-dpdp-grievance"><?php esc_html_e( 'Grievance / contact information', 'privacypress' ); ?></label></th>
