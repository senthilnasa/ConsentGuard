...
$pcm_positions  = array(
	'header' => __( 'Header', 'privacypress' ),
	'body'   => __( 'Body (after opening body tag)', 'privacypress' ),
	'footer' => __( 'Footer', 'privacypress' ),
);

Admin::maybe_notice();

if ( ! current_user_can( 'unfiltered_html' ) ) {
	echo '<div class="notice notice-warning"><p>' . esc_html__( 'Your account lacks the unfiltered_html capability: script tags you save will be stripped to safe HTML. Ask a super administrator to add executable scripts.', 'privacypress' ) . '</p></div>';
}
?>
<h1><?php esc_html_e( 'Script Manager', 'privacypress' ); ?></h1>
<p><?php esc_html_e( 'Custom scripts are printed in consent-blocked form and execute only after the visitor grants the selected category. Empty the name to delete a script.', 'privacypress' ); ?></p>
...
foreach ( $pcm_rows as $pcm_i => $pcm_script ) :
	...
	<div class="pcm-card pcm-script-row">
		<h2><?php echo $pcm_is_new ? esc_html__( 'Add Script', 'privacypress' ) : esc_html( $pcm_script['name'] ); ?></h2>
		<input type="hidden" name="pcm[custom_scripts][<?php echo esc_attr( $pcm_i ); ?>][id]" value="<?php echo esc_attr( $pcm_script['id'] ); ?>" />
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Name', 'privacypress' ); ?></th>
				<td><input type="text" class="regular-text" name="pcm[custom_scripts][<?php echo esc_attr( $pcm_i ); ?>][name]" value="<?php echo esc_attr( $pcm_script['name'] ); ?>" placeholder="<?php esc_attr_e( 'e.g. Facebook Pixel', 'privacypress' ); ?>" /></td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Category', 'privacypress' ); ?></th>
				<td>
			...
			<tr>
				<th scope="row"><?php esc_html_e( 'Enabled', 'privacypress' ); ?></th>
			...
			<tr>
				<th scope="row"><?php esc_html_e( 'Script', 'privacypress' ); ?></th>
				<td>
					...
					<p class="description"><?php esc_html_e( 'Inline snippets, external script tags and tracking pixels are supported.', 'privacypress' ); ?></p>
				</td>
			</tr>
		</table>
	</div>
