...
Admin::maybe_notice();
?>
<h1><?php esc_html_e( 'Plugin Conflicts', 'consentguard' ); ?></h1>
<p><?php esc_html_e( 'Plugins below may inject the same trackers this plugin manages, which can cause duplicate data collection. Conflicts are never resolved by deactivating a plugin — only the specific tracking output is suppressed, and only where the other plugin provides a supported way to do so.', 'consentguard' ); ?></p>

<?php if ( empty( $pcm_conflicts ) ) : ?>
	<div class="notice notice-success inline"><p><?php esc_html_e( 'No potential tracking conflicts detected.', 'consentguard' ); ?></p></div>
<?php endif; ?>
...
<?php foreach ( $pcm_conflicts as $pcm_conflict ) : ?>
	<div class="pcm-card <?php echo $pcm_conflict['ignored'] ? 'pcm-muted' : ''; ?>">
		<h2>
			...
			<?php esc_html_e( 'Potential Tracking Conflict', 'consentguard' ); ?> — <?php echo esc_html( $pcm_conflict['plugin_name'] ); ?>
		</h2>
		<p>
			<?php
			printf(
				/* translators: 1: service name, 2: plugin name */
				esc_html__( '%1$s may be injected by %2$s. ConsentGuard is also configured to manage %1$s.', 'consentguard' ),
		...
		<p>
			<?php if ( ! $pcm_conflict['mitigated'] && 'supported' === $pcm_conflict['mitigation'] ) : ?>
				<a class="button button-primary" href="<?php echo esc_url( $pcm_action_url( $pcm_conflict['id'], 'mitigate' ) ); ?>">
					...
					printf(
						...
						esc_html__( 'Disable %2$s output from %1$s', 'consentguard' ),
						esc_html( $pcm_conflict['plugin_name'] ),
			...
			<?php elseif ( $pcm_conflict['mitigated'] ) : ?>
				<a class="button" href="<?php echo esc_url( $pcm_action_url( $pcm_conflict['id'], 'unmitigate' ) ); ?>"><?php esc_html_e( 'Re-enable', 'consentguard' ); ?></a>
			<?php endif; ?>

			<?php if ( $pcm_conflict['ignored'] ) : ?>
				<a class="button" href="<?php echo esc_url( $pcm_action_url( $pcm_conflict['id'], 'unignore' ) ); ?>"><?php esc_html_e( 'Stop ignoring', 'consentguard' ); ?></a>
			<?php else : ?>
				<a class="button" href="<?php echo esc_url( $pcm_action_url( $pcm_conflict['id'], 'ignore' ) ); ?>"><?php esc_html_e( 'Ignore', 'consentguard' ); ?></a>
			<?php endif; ?>
		</p>
	</div>
<?php endforeach; ?>
