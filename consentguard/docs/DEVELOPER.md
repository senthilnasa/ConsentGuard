...
```text
consentguard.php     bootstrap, autoloader, activation hooks
includes/
...
- Gutenberg: `consentguard/privacy-settings` (button, optional label) and
  `consentguard/cookie-table` (live inventory table) — both dynamic.
...
- WP-CLI: `wp consentguard stats`, `wp consentguard scan`,
  `wp consentguard cleanup`, `wp consentguard export --file=consents.csv`.
