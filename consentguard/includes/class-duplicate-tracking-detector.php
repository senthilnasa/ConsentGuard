...
class Duplicate_Tracking_Detector {
	...
	public function signatures() {
		return array(
			'ga4'        => array(
				'label'    => __( 'Google Analytics 4', 'consentguard' ),
				'patterns' => array(
			...
			'gtm'        => array(
				'label'    => __( 'Google Tag Manager', 'consentguard' ),
				'patterns' => array(
			...
			'clarity'    => array(
				'label'    => __( 'Microsoft Clarity', 'consentguard' ),
				'patterns' => array(
			...
			'cloudflare' => array(
				'label'    => __( 'Cloudflare Web Analytics', 'consentguard' ),
				'patterns' => array(
					'#static\.cloudflareinsights\.com/beacon(?:\.min)?\.js#i',
				),
			),
			'meta_pixel' => array(
				'label'    => __( 'Meta Pixel', 'consentguard' ),
				'patterns' => array(
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Could not fetch the homepage for scanning: %s', 'consentguard' ),
		...
		if ( $code < 200 || $code >= 400 ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Homepage scan returned HTTP status %d.', 'consentguard' ),
					$code
				)
			);
		}
	...
	private function attribute_sources( $service, array $active_plugins ) {
		...
		if ( ! empty( $ours[ $service ] ) ) {
			$sources[] = __( 'ConsentGuard', 'consentguard' );
		}
