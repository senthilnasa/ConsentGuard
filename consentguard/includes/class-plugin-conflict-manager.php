...
class Plugin_Conflict_Manager {
	...
	public static function service_labels() {
		return array(
			...
			'meta_pixel' => __( 'Meta Pixel', 'consentguard' ),
		);
	}
		...
		foreach ( $this->detector->detect_active() as $basename => $plugin ) {
			foreach ( $plugin['services'] as $service ) {
				...
				$conflicts[] = array(
					...
					'note'          => $supported
						? __( 'Duplicate output can be suppressed using this plugin\'s supported hooks.', 'consentguard' )
						: __( 'Automatic conflict resolution is not available. Please disable this tracking feature in the other plugin\'s own settings.', 'consentguard' ),
				);
			}
		}
	...
	public function apply_mitigation( $conflict_id ) {
		...
		if ( ! $conflict ) {
			return new \WP_Error( 'pcm_unknown_conflict', __( 'Unknown conflict.', 'consentguard' ) );
		}
		if ( 'supported' !== $conflict['mitigation'] ) {
			return new \WP_Error(
				'pcm_manual_only',
				__( 'Automatic conflict resolution is not available for this plugin. Please disable the tracking feature manually in that plugin\'s settings.', 'consentguard' )
			);
		}
