 ...
 *
 * - consentguard/privacy-settings: a button/link that opens the consent
 *   preferences modal (same behaviour as the [pcm_privacy_settings]
 *   shortcode).
 * - consentguard/cookie-table: renders the live cookie inventory as a
...
class Blocks {
	...
	public function render_cookie_table() {
		...
		foreach ( $categories as $slug => $category ) {
			...
			$out .= '<table><thead><tr><th>' . esc_html__( 'Cookie', 'consentguard' ) . '</th><th>' . esc_html__( 'Duration', 'consentguard' ) . '</th><th>' . esc_html__( 'Description', 'consentguard' ) . '</th></tr></thead><tbody>';
